...
ZONE zones[W_WIDTH / ZONE_SEC][W_HEIGHT / ZONE_SEC];
...
void process_packet(int c_id, char* packet)
{
	...
	case CS_LOGIN: {
		...
		zones[clients[c_id].x / ZONE_SEC][clients[c_id].y / ZONE_SEC].ADD(c_id);
		...
		ZONE* my_zone[4];
		zones[clients[c_id].x / ZONE_SEC][clients[c_id].y / ZONE_SEC];
		int zoneX = clients[c_id].x / ZONE_SEC;
		int zoneY = clients[c_id].y / ZONE_SEC;
		my_zone[0] = &zones[zoneX][zoneY];
		int zoneid = 1;
		if(clients[c_id].x % ZONE_SEC < 10 && zoneX > 1)
		{ 
			zoneid++;
			my_zone[zoneid] = &zones[zoneX - 1][zoneY];
		}
	...
	case CS_MOVE: {
		...
		zones[clients[c_id].x / ZONE_SEC][clients[c_id].y / ZONE_SEC].DEL(c_id);
		zones[x / ZONE_SEC][y / ZONE_SEC].ADD(c_id);
		clients[c_id].x = x;
		clients[c_id].y = y;
